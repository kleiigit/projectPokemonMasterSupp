...
namespace ProjetoPokemon.Entities
{
    internal class BoxPokemon
    {
        ...
        public void DrawItemCard(int n)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ItemCard? card = MasterGame.DrawItemCard();
                if(card == null)
                {
                    Console.WriteLine("Item deck empty!");
                    continue;
                }
        ...
        public ItemCard? SelectItem(TypeItemCard type)
        {
            ...
            options.Add("No use card");
            // Exibe o menu
            int choice = ConsoleMenu.ShowMenu(ConsoleColor.Blue ,options, $"{Nickname}: Choose a {type.ToString()} Item Card");
        ...
        public static BoxPokemon ChooseProfileTrainer()
        {
            int index = ConsoleMenu.ShowMenu(ConsoleColor.Green, DataLists.AllProfiles.Select(m => m.ToString()).ToList(), "Select a Trainer Profile");
            return DataLists.AllProfiles[index];
        }
        ...
        public string DescriptionBox()
        {
            Console.Clear();
            string description = "\nPokemon: \n";
            foreach (var pokemon in ListPokemon.OrderBy(p => p.LevelPokemon()).OrderByDescending(p => p.LevelPokemon()))
            {
                description += pokemon.ToString() + "\n";
            }
            description += "\nItem: \n";
            foreach (var item in ListCards.OrderBy(p => p.Key.Type))
            {
                description += $"{item.Value} = {item.Key.Name}";
                description += "\n";
            }
        ...
        {
            return $"{Nickname} - Pokemon: {ListPokemon.Count}, Item: {ListCards.Values.Sum()}, Badge: {Badges.Count}";
        }
    }
}
